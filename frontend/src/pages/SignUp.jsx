...
function SignUp() {
  ...
  return (
    <div className='w-full min-h-screen flex items-center justify-center relative bg-gradient-to-br from-gray-950 to-black p-4'>
        ...
        <form action="" className='max-w-md w-full p-8 bg-black/40 backdrop-blur-xl rounded-xl shadow-2xl flex flex-col items-center gap-6 border border-white/10' onSubmit={handleSignUP}>
            ...
            <div className='w-full flex flex-col gap-2'>
                <label htmlFor="name" className='text-lg text-white font-medium'>Username</label>
                <input 
                    type="text" 
                    id='name' 
            ...
            <button className='w-full py-3 bg-gradient-to-r from-red-600 to-red-800 text-white text-xl font-semibold rounded-lg shadow-lg hover:from-red-700 hover:to-red-900 transition-all duration-300 transform hover:scale-105 mt-6' disabled={loading}>{loading?"Loading...":"Sign Up"}</button>
            <p className='text-white text-lg text-center'>Already have an account? <span className='text-red-500 cursor-pointer font-semibold hover:underline transition-colors duration-200' onClick={()=>navigate("/login")}>Login</span>
            </p>
        </form>
    </div>
  )
